...
def Xs_to_indexes(Xs):
    """
    Converts a list of array (some of which can be None)
    to useful arrays and relevant indexes

    Parameters
    ----------
    Xs: list (some items can be None)

    Returns
    -------
    Xs_useful: list of arrays
    indexes: np array
    """
    indexes = [i for i, x in enumerate(Xs) if x is not None]
    Xs_useful = [x for x in Xs if x is not None]
    return Xs_useful, indexes


def indexes_to_Xs(Xs_useful, indexes, n_views):
    """
    Converts arrays and corresponding indexes to
    list of array (some of which can be None)
    ...
    Xs: list (some items can be None)
    """
    indexes_dict = {i: k for k, i in enumerate(indexes)}
    return [
        Xs_useful[indexes_dict[i]] if i in indexes else None
