...
class KCCA(BaseEmbed):
    ...
    def fit(self, Xs):
        ...
        self.Kx = _make_kernel(
            x, x, self.ktype, self.constant, self.degree, self.sigma
        )
        self.Ky = _make_kernel(
            y, y, self.ktype, self.constant, self.degree, self.sigma
        )
        ...
        D = np.r_[
            np.c_[self.Kx + self.reg * Id, Z],
            np.c_[Z, self.Ky + self.reg * Id],
        ]
        ...
        #     [KyKx   0 ]
        # D = []
        #R = np.r_[np.c_[Z, self.Ky @ self.Kx], np.c_[self.Kx @ self.Ky, Z]]
        #D = np.r_[
        #    np.c_[self.Kx @ self.Kx + self.reg * Id, Z],
        #    np.c_[Z, self.Ky @ self.Ky + self.reg * Id],
        #]

        # # R\alpha = \lambda D \beta
        #betas, alphas = linalg.eigh(
        #    0.5 * (R + R.T), 0.5 * (D + D.T)
...
def _center_norm(x):
    ...
    # TODO learn mean/std
    return x  # @np.sqrt(np.diag(np.divide(1, np.diag(x.T@x))))


def _make_kernel(X, Y, ktype, constant=0.0, degree=2.0, sigma=1.0):
    Nl = len(X)
    Nr = len(Y)
