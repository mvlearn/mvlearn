...
class MultiviewSphericalKMeans(MultiviewKMeans):
    ...
    def _em_step(self, X, partition, centroids):
        ...
        '''
        sk_means = SphericalKMeans(n_clusters=self.n_clusters,
                                   max_iter=1, n_init=1, init=centroids)
        new_parts = kmeans.fit_predict(X)
        new_centers = cluster_centers_
        
        o_funct = np.sum(min_dists)
        o_funct = sk_means.intertia_
