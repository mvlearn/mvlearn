...
# approximation of the Cholesky decomposition of the kernel matrix. This reduces
# storage requirements from O(n^2) to O(nm), where n is the number of subjects
# (rows) and m is the rank of the kernel matrix. This also reduces the run-time
# from O(n^3) to O(nm^2).
...
# magnitude decrease in run-time compared to a rank of 100 (full decomposition).
