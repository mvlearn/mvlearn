...
Single view ICA decomposes a dataset :math:`X` as :math:`X = S \\times A^{\\top}`, where
:math:`S` are the independent sources (meaning that the columns of :math:`S` are
independent), and :math:`A` is the mixing matrix.

In group ICA, we have several views :math:`Xs = [X_1, \\dots, X_n]`. Each view is obtained as

.. math::

	X_i \\simeq S \\times A_i.T

so the views share the same sources :math:`S`, but have different mixing matrices
:math:`A_i`. It is a powerful tool for group inference, as it allows to extract signals
that are comon across views.
...
# -------------------------
#
# We see they look pretty good! We can also wheck that it has correctly predicted each
# mixing matrix. The estimated mixing matrices are stored in the
# `.individual_mixing_` attribute.
#
# If :math:`\tilde{A}` is the estimated mixing matrix and :math:`A` is the true mixing
# matrix, we can look at :math:`\tilde{A}^{-1}A`. It should be close to a scale and
