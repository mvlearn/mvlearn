...
def test_info_view_not_valid(small_data):
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=2, info_view=-1)
        spectral.fit_predict(small_data)
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=2, info_view=6)
        spectral.fit_predict(small_data)
...
def data():
    ...
    spectral = MultiviewCoRegSpectralClustering(n_clusters, random_state=RANDOM_STATE)
...
def test_fit_predict_max_iter(data):
    ...
    max_iter = 5
    n_clusts = data['n_clusters']
    spectral = MultiviewCoRegSpectralClustering(n_clusts,
