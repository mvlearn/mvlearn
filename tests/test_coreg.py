...
def test_n_clusters_not_positive_int():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=-1)
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=0)

def test_n_views_not_positive_int():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=5, n_views=-1)
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=5, n_views=0)

def test_random_state_not_convertible():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=5, random_state='ab')


def test_info_view_not_valid():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=2, n_views=5, info_view=-1)
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_clusters=2, n_views=5, info_view=6)
...
def test_samples_not_n_views():
    with pytest.raises(ValueError):
        ...
        spectral = MultiviewCoRegSpectralClustering(n_views=3, random_state=RANDOM_STATE)
        spectral.fit_predict([view1, view2])

def test_max_iter_not_positive_int():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(max_iter=-1)
        
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(max_iter=0)

def test_n_init_not_positive_int():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_init=-1)
        
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(n_init=0)

def test_not_valid_affinity():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(affinity='What')
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(affinity=None)

def test_gamma_not_positive_float():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(gamma=-1.5)
        
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(gamma=0)
        
def test_n_neighbors_not_positive_int():
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(affinity='nearest_neighbors', n_neighbors=-1)
        
    with pytest.raises(ValueError):
        spectral = MultiviewCoRegSpectralClustering(affinity='nearest_neighbors', n_neighbors=0) 
...
def data():
    ...
    spectral = MultiviewCoRegSpectralClustering(n_clusters, n_views=n_views, random_state=RANDOM_STATE)
...
def test_affinity_mat_rbf2(data):
    ...
    gamma = 1
    spectral = MultiviewCoRegSpectralClustering(n_views=n_views,
...
def test_fit_predict_default(data):
    ...
    spectral = MultiviewCoRegSpectralClustering(2, random_state=RANDOM_STATE)
...
def test_fit_predict_max_iter(data):
    ...
    max_iter = 5
    n_clusts = data['n_clusters']
    spectral = MultiviewCoRegSpectralClustering(n_clusts, n_views=n_views,
